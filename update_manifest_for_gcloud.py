...
from bdcat_ingest import get_gcloud_checksums_and_metadata_for_dict
...
def main():
	...
	get_gcloud_checksums_and_metadata_for_dict(updated_od, args.checksum_threads, args.bucket)
	global out_file
	out_file = get_receipt_manifest_file_pointer_for_bucket(args.bucket)	
	update_manifest_file(out_file, od)				
	out_file.close()
	upload_manifest_file_to_gcs_bucket(out_file.name, args.bucket)
	print("Done. Receipt manifest located at", out_file.name)

def parse_args():
	parser = argparse.ArgumentParser(description='Update TSV file for GCS Bucket.')
