#!/usr/bin/env python3
#
# usage: process_dicom_in_gcs_from_files.py --study_id STUDY_ID --consent_group CONSENT_GROUP
#				--gcs_bucket GCS_BUCKET --gcs_user_project GCS_USER_PROJECT
#				--output_manifest_file OUTPUT_MANIFEST_FILE --directory DIRECTORY
#              [-h] [--test] [--resume] 
...
#time python process_dicom_in_gcs_from_files.py --gcs_bucket idc-tcia-tcga-read --gcs_user_project nimbus-fhir-test --study_id idc-tcia-tcga-read --consent_group oa --directory idc-tcia-tcga-read
...
from bdcat_ingest_dicom import generate_dict_from_file_directory_for_dicom
...
def main():
	...
	if (not gcs_bucket_writeable(upload_gcs_bucket_name)):
		...
		exit()
	od = generate_dict_from_file_directory_for_dicom(args.gcs_bucket, args.study_id, args.consent_group) 
	global out_file
